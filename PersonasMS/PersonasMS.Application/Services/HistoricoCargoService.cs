...
{
    public class HistoricoCargoService : IBaseService<HistoricoCargoCreateDto, HistoricoCargoDto>
    {
        private readonly IBaseRepository<HistoricoCargo> _baseRepository;
        private readonly IMapper _mapper;

        public HistoricoCargoService(IBaseRepository<HistoricoCargo> baseRepository, IMapper mapper)
        {
            _baseRepository = baseRepository;
            _mapper = mapper;
        }

        public (bool status, int id) Post(HistoricoCargoCreateDto entity)
        {
            var obj = _mapper.Map<HistoricoCargo>(entity);
            var result = _baseRepository.Insert(obj);
            return (result, obj.Id);
        }

        public async Task<bool> PutAsync(HistoricoCargoDto entity, int id)
        {
        ...
        public async Task<IEnumerable<HistoricoCargoDto>> GetAllAsync(int page, int limit, string orderBy, bool ascending = true)
        {
            var result = await _baseRepository.GetAllAsync(page, limit, orderBy, ascending).ConfigureAwait(false);
            return _mapper.Map<IEnumerable<HistoricoCargoDto>>(result);
        }

        public async Task<HistoricoCargoDto> GetByIdAsync(int id)
        {
            var result = await _baseRepository.FirstOrDefaultAsync(x => x.Id == id).ConfigureAwait(false);
            return _mapper.Map<HistoricoCargoDto>(result);
        }
