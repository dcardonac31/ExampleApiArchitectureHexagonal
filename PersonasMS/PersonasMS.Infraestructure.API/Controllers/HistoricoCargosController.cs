...
namespace PersonasMS.Infraestructure.API.Controllers
{
    ...
    [ApiController]
    public class HistoricoCargosController : ControllerBase
    {
        private readonly ILogger<HistoricoCargosController> _logger;
        private readonly IBaseService<HistoricoCargoCreateDto, HistoricoCargoDto> _service;
        private readonly IPersonaService _personaService;
        private readonly IMapper _mapper;

        public HistoricoCargosController(ILogger<HistoricoCargosController> logger, IBaseService<HistoricoCargoCreateDto, HistoricoCargoDto> service, IPersonaService personaService, IMapper mapper)
        {
            _logger = logger;
            _service = service;
        ...
        // POST api/<HistoricoCargosController>
        [HttpPost]
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        [ProducesResponseType(typeof(ResponseObjectDto<string>), (int)HttpStatusCode.Created)]
        [Produces(MediaTypeNames.Application.Json, Type = typeof(HistoricoCargo))]
        public IActionResult Post([FromBody] HistoricoCargoCreateDto request)
        {
            _logger.LogInformation(nameof(HistoricoCargosController));
            ...
            if (status)
            {
                return Ok(new ResponseObjectDto<int>
                {
                    ...
                    Status = status,
                    Message = "Historico Cargo insertado con éxito.",
            ...
            else
            {
                return BadRequest(new ResponseObjectDto<int>
                {
                    ...
                    Status = status,
                    Message = "Historico cargo no insertado con éxito.",
                    Data = status ? id : default
                });
            }
        }

        // PUT api/<HistoricoCargosController>/5
        [HttpPut("{id}")]
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        [ProducesResponseType((int)HttpStatusCode.NotFound)]
        [ProducesResponseType(typeof(ResponseObjectDto<bool>), (int)HttpStatusCode.OK)]
        [Produces(MediaTypeNames.Application.Json, Type = typeof(HistoricoCargo))]
        public async Task<IActionResult> PutAsync(int id, [FromBody] HistoricoCargoDto request)
        {
            _logger.LogInformation(nameof(HistoricoCargosController));
            ...
            if (status)
            {
                return Ok(new ResponseObjectDto<bool>
                {
                    HttpStatusCode = HttpStatusCode.OK,
                    Status = status,
                    Message = "Historico cargo actualizado con éxito.",
                    Data = status
                });
            }
            else
            {
                return BadRequest(new ResponseObjectDto<bool>
                {
                    HttpStatusCode = HttpStatusCode.BadRequest,
                    Status = status,
                    Message = "Historico cargo no actualizada con éxito.",
                    Data = status
                });
            }
        ...
        // GET: api/<HistoricoCargosController>
        [HttpGet("{page:int}/{limit:int}")]
        [ProducesResponseType(typeof(ResponseObjectDto<IEnumerable<HistoricoCargo>>), (int)HttpStatusCode.OK)]
        public async Task<IActionResult> GetAllAsync(int? page, int? limit)
        {
            ...
            var resultDto = result as HistoricoCargoDto[] ?? result.ToArray();

            var response = new ResponseObjectDto<IEnumerable<HistoricoCargoDto>>
            {
        ...
        // GET api/<HistoricoCargosController>/5
        [HttpGet("{id}")]
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        [ProducesResponseType(typeof(ResponseObjectDto<HistoricoCargoDto>), (int)HttpStatusCode.OK)]
        public async Task<IActionResult> GetByIdAsync(int id)
        {
            ...
            var response = new ResponseObjectDto<HistoricoCargoDto>
            {
        ...
        // DELETE api/<HistoricoCargosController>/5
        [HttpDelete("{id}")]
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        [ProducesResponseType((int)HttpStatusCode.NotFound)]
        [ProducesResponseType(typeof(ResponseObjectDto<bool>), (int)HttpStatusCode.OK)]
        ...
        [HttpPost("insert-data-excel-historico-cargo")]
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        [ProducesResponseType(typeof(ResponseObjectDto<string>), (int)HttpStatusCode.Created)]
        [Produces(MediaTypeNames.Application.Json, Type = typeof(HistoricoCargo))]
        public async Task<IActionResult> InsertDataExcelHistoricoCargo([FromForm] IFormFile filePath)
        {
            _logger.LogInformation(nameof(HistoricoCargosController));
            _logger.LogInformation(nameof(InsertDataExcelHistoricoCargo));
            ...
            using (var file = new FileStream(filePath.FileName, FileMode.Open, FileAccess.Read))
            {
                ...
                for (var i = sheet.FirstRowNum; i <= sheet.LastRowNum; i++)
                {
                    ...
                    if (i >= rowInitial)
                    {
                        ...
                        var cargoId = ExcelValidation.GetCellValue<int>(row.GetCell(1), 1);
                        var fechaAsignacion = ExcelValidation.GetCellValue<DateTime>(row.GetCell(2), new DateTime(1900, 1, 1));
                        var fechaDesasignacion = ExcelValidation.GetCellValue<DateTime>(row.GetCell(3), new DateTime(1900, 1, 1));

                        var request = new HistoricoCargoCreateDto();
                ...
                if (contRows > 0)
                {
                    return Ok(new ResponseObjectDto<int>
                    {
                        ...
                        Message = $"Cantidad de registros de HistoricoCargo insertados con éxito: {contRows}",
                        Data = contRows
                    });
                }
                else
                {
                    return BadRequest(new ResponseObjectDto<int>
                    {
                        HttpStatusCode = HttpStatusCode.BadRequest,
                        Status = false,
                        Message = "Error en carga de archivo de HistoricoCargo.",
                        Data = contRows
                    });
                }
            }
